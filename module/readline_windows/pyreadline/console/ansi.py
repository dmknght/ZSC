                ...
                for part in parts:
                    if part == "0":  # No text attribute
                        ...
                        attr.bold = True
                            ...
                            part) == 2 and "30" <= part <= "37":  # set foreground color
                        attr.color = trtable[int(part) - 30]
                    elif len(
                            part) == 2 and "40" <= part <= "47":  # set background color
        ...
        if m:
            for part in m.group(1).split(";"):
                ...
                if part == "1":  # switch on bold (i.e. intensify foreground color)
                    ...
                    attr = (attr & ~0x07) | (
                        (part & 0x1) << 2) | (part & 0x2) | ((part & 0x4) >> 2)
                elif len(
                        part) == 2 and "40" <= part <= "47":  # set background color
                    part = int(part) - 40
                    # we have to mirror bits
                    attr = (attr & ~0x70) | ((part & 0x1) << 6) | (
                        (part & 0x2) << 4) | ((part & 0x4) << 2)
