...
#        self._bind_key('Control-e', self.emacs)
    ...
    def _vi_do_backspace(self, vi_cmd):
        if self.vi_is_insert_mode or (self._vi_command and
                                      self._vi_command.is_search):
    ...
    def vi_save_line(self):
        ...
        tpl_undo = (self.l_buffer.point,
                    self.l_buffer.line_buffer[:], )
        if not self._vi_undo_stack or self._vi_undo_stack[
                self._vi_undo_cursor][1] != tpl_undo[1]:
    ...
    def vi_search_again_forward(self):
        self.vi_search(list(range(self._vi_search_position + 1, len(
...
#             #overwrite in l_buffer obj
    ...
    def key_p(self, char):
        ...
        self.readline.vi_save_line()
        self.readline.l_buffer.point += 1
        self.readline.l_buffer.insert_text(self.readline._vi_yank_buffer *
                                           self.get_multiplier())
        self.readline.l_buffer.point -= 1
        self.state = _VI_END
...
def vi_is_word(char):
    log('xx vi_is_word: type(%s), %s' % (type(char),
                                         char, ))
