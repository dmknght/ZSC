...
class NotEmacsMode(basemode.BaseMode):
    ...
    def _readline_from_keyboard(self):
        ...
        while 1:
            ...
            dispatch_func = self.key_dispatch.get(event.keyinfo,
                                                  self.self_insert)
    ...
    def readline(self, prompt=''):
        ...
        if len(self.paste_line_buffer) > 0:
            self.l_buffer = lineobj.ReadlineTextBuffer(self.paste_line_buffer[
                0])
            self._update_line()
...
#########  History commands
    ...
    def self_insert(self, e):  # (a, b, A, 1, !, ...)
        '''Insert yourself. '''
        if ord(e.
               char) != 0:  #don't insert null character in buffer, can happen with dead keys.
            self.insert_text(e.char)
