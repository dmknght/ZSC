...
class Encoder(BaseModule):
    ...
    def encode(self, shellcode):
        ...
        while True:
            ecx_1 = binascii.b2a_hex((''.join(random.choice(chars) for i in range(4))).encode('latin-1')).decode(
                'latin-1')
        ...
        for char in middle.rsplit('\n'):
            while True:
                if 'push $0x' in char:
                    ...
                    if '00' not in str(ebx_1) and '00' not in str(ebx_2) and len(ebx_2) >= 7 and len(
