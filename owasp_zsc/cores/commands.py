...
def _help_cli(help_cli_items):
    write('\n')
    for item in help_cli_items:
        items = ''
        for i in item[0]:
            items += str(i) + ', '
        items = items[:-2]
...
def show_samples(args_cmd_samples):
    write('\n')
    for item in args_cmd_samples:
        info(item + '\n')
    write('\n')


def show_payloads(usr_commands, check_payload):
    shellcodes = usr_commands['shellcode'][1]['generate']
    obfuscate = usr_commands['obfuscate'][1]
    payloads = []
    for a in shellcodes:
